<?php
	...
	$sSQL .= " street_place_id, rank_address, rank_search, get_searchrank_label(rank_search) as rank_search_label, indexed, get_name_by_language(name,$sLanguagePrefArraySQL) as localname, ";
	$sSQL .= " ST_GeometryType(geometry) in ('ST_Polygon','ST_MultiPolygon') as isarea,ST_GeometryType(geometry) as geotype, array_upper(name, 1) as numnames, ST_Y(ST_Centroid(geometry)) as lat,ST_X(ST_Centroid(geometry)) as lon ";
