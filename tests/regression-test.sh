...
function setup_db {
    ...
    createdb -E UTF8 -O openstreetmap osm2pgsql-test
...
function teardown_db {
    ...
    createdb -E UTF8 -O openstreetmap osm2pgsql-test # Create db, so that we can run the drop tablespace command against
    psql -c "DROP TABLESPACE tablespacetest" -d osm2pgsql-test
...
function test_osm2pgsql_slim {
    ...
    echo "Number of points imported"
    psql -c "SELECT count(*) FROM planet_osm_point;" -t -d osm2pgsql-test || true
    echo "Number of lines imported"
    psql -c "SELECT count(*) FROM planet_osm_line;" -t -d osm2pgsql-test || true
    echo "Number of roads imported"
    psql -c "SELECT count(*) FROM planet_osm_roads;" -t -d osm2pgsql-test || true
    echo "Number of polygon imported"
    psql -c "SELECT count(*) FROM planet_osm_polygon;" -t -d osm2pgsql-test || true
    echo "Number of nodes imported"
    psql -c "SELECT count(*) FROM planet_osm_nodes;" -t -d osm2pgsql-test || true
    echo "Number of ways imported"
    psql -c "SELECT count(*) FROM planet_osm_ways;" -t -d osm2pgsql-test || true
    echo "Number of relations imported"
    psql -c "SELECT count(*) FROM planet_osm_rels;" -t -d osm2pgsql-test || true
...
test_osm2pgsql_slim "-p planet_osm2 -S default.style -C 100"
test_osm2pgsql_slim "-O gazetteer -S default.style -C 100"
test_osm2pgsql_slim "--bbox -90.0,-180.0,90.0,180.0 -O gazetteer -S default.style -C 100"
