               ...
               ...)
{
    ...
    while (1) {
        ...
        if ((nsql = static_cast<char *>(realloc(sql, size))) == nullptr) {
            ...
            throw std::runtime_error(
                "Memory re-allocation failed in pgsql_exec");
        } else {
            sql = nsql;
        }
    }
    ...
    if (PQresultStatus(res.get()) != expect) {
        std::string err_msg =
            (boost::format("%1% failed: %2%") % sql % PQerrorMessage(sql_conn))
                .str();
        free(sql);
                    ...
                    std::string const &sql)
{
    ...
    case 0:
        throw std::runtime_error(
            (boost::format("%1% - bad result during COPY, data %2%") % context %
    ...
    case -1:
        throw std::runtime_error(
            (boost::format("%1%: %2% - bad result during COPY, data %3%") %
             PQerrorMessage(sql_conn) % context % sql)
                .str());
        break;
