...
TRAMPOLINE(app_geometry_gc, geometry_gc)
...
TRAMPOLINE(geom_transform, transform)
TRAMPOLINE(geom_split_multi, split_multi)
TRAMPOLINE(geom_simplify, simplify)
...
static char const *const osm2pgsql_geometry_name = "osm2pgsql.Geometry";
...
geom::geometry_t *unpack_geometry(lua_State *lua_state) {
    void *user_data = lua_touserdata(lua_state, -1);

    if (user_data == nullptr || !lua_getmetatable(lua_state, -1)) {
        throw std::runtime_error{"First parameter must be a geometry."};
    }

    luaL_getmetatable(lua_state, osm2pgsql_geometry_name);
...
void create_lua_geometry_object(lua_State *lua_state, CREATE &&create)
{
    void *ptr = lua_newuserdata(lua_state, sizeof(geom::geometry_t));
    new (ptr) geom::geometry_t{};
    std::forward<CREATE>(create)(static_cast<geom::geometry_t *>(ptr));
...
int output_flex_t::geom_centroid()
{
    if (lua_gettop(lua_state()) > 1) {
...
int output_flex_t::geom_transform()
{
    if (lua_gettop(lua_state()) != 2) {
        throw std::runtime_error{
            "One parameter needed for transform(), the target srid."};
    }
...
#endif
        throw std::runtime_error{
            "transform() srid parameter has to be an integer."};
    }
...
// XXX this is inefficient, because we are copying the whole geometry and
// creating a large Lua array. Better implement this with an iterator?
int output_flex_t::geom_split_multi()
{
    if (lua_gettop(lua_state()) > 1) {
...
int output_flex_t::geom_simplify()
{
    if (lua_gettop(lua_state()) != 2) {
        throw std::runtime_error{
            "simplify() function needs one parameter: tolerance."};
    }

    if (!lua_isnumber(lua_state(), -1)) {
        throw std::runtime_error{
            "simplify() tolerance parameters has to be a number."};
    }
...
int output_flex_t::app_geometry_gc()
{
    ...
    lua_pop(lua_state(), 1); // pop parameter from stack
 ...
 */
static void init_geometry_class(lua_State *lua_state)
{
    if (luaL_newmetatable(lua_state, osm2pgsql_geometry_name) != 1) {
        throw std::runtime_error{"Internal error: Lua newmetatable failed."};
    }

    luaX_add_table_func(lua_state, "__gc", lua_trampoline_app_geometry_gc);
    luaX_add_table_func(lua_state, "__len", lua_trampoline_geom_len);
    luaX_add_table_func(lua_state, "__tostring", lua_trampoline_geom_tostring);
    lua_pushvalue(lua_state, -1);
    lua_setfield(lua_state, -2, "__index");
    ...
    luaX_add_table_func(lua_state, "geometrytype",
                        lua_trampoline_geom_geometrytype);
    luaX_add_table_func(lua_state, "centroid", lua_trampoline_geom_centroid);
    luaX_add_table_func(lua_state, "transform", lua_trampoline_geom_transform);
    luaX_add_table_func(lua_state, "split_multi",
                        lua_trampoline_geom_split_multi);
    luaX_add_table_func(lua_state, "simplify", lua_trampoline_geom_simplify);
    lua_pop(lua_state, 1); // __index
}
