...
void init_params(params_t *params, options_t const &options)
{
    ...
    params->set_string("prefix", options.prefix);
    params->set_string("schema", schema);
    params->set_string("unlogged", options.droptemp ? "UNLOGGED" : "");
    params->set_string("data_tablespace",
                       tablespace_clause(options.tblsslim_data));
    params->set_string("index_tablespace",
                       tablespace_clause(options.tblsslim_index));
    params->set_int64("way_node_index_id_shift", 5);

    if (options.tblsslim_index.empty()) {
        params->set_string("using_tablespace", "");
    } else {
        params->set_string("using_tablespace",
                           "USING INDEX TABLESPACE " + options.tblsslim_index);
    }
    ...
    if (options.extra_attributes) {
        params->set_string("attribute_columns_definition",
                           " created timestamp with time zone,"
                           " version int4,"
                           " changeset_id int4,"
                           " user_id int4,");
        params->set_string("attribute_columns_use",
                           ", EXTRACT(EPOCH FROM created) AS created, version, "
                           "changeset_id, user_id, u.name");
        params->set_string("users_table_access",
