...
TEST_CASE("multi backend tag import")
{
    ...
    REQUIRE(1 == conn.get_count("test_points_1", "foo IS NULL and bar = 'n1' AND baz IS NULL"));
    REQUIRE(1 == conn.get_count("test_points_1", "foo IS NULL and bar = 'n2' AND baz IS NULL"));
    REQUIRE(1 == conn.get_count("test_line_1", "foo IS NULL and bar = 'w1' AND baz IS NULL"));
    REQUIRE(1 == conn.get_count("test_polygon_1", "foo IS NULL and bar = 'w2' AND baz IS NULL"));

    // Check that the second table also got the right transform
    REQUIRE(1 == conn.get_count("test_points_2", "foo IS NULL and bar IS NULL AND baz = 'n1'"));
    REQUIRE(1 == conn.get_count("test_points_2", "foo IS NULL and bar IS NULL AND baz = 'n2'"));
    REQUIRE(1 == conn.get_count("test_line_2", "foo IS NULL and bar IS NULL AND baz = 'w1'"));
    REQUIRE(1 == conn.get_count("test_polygon_2", "foo IS NULL and bar IS NULL AND baz = 'w2'"));
}
